...
_NUBANK_API = "https://boards-api.greenhouse.io/v1/boards/nubank/jobs?content=true"
...
async def fetch_nubank_jobs(_keywords: list[str], _cities: list[str]) -> list[dict]:
    jobs: list[dict] = []
    ...
    try:
        async with httpx.AsyncClient(headers=_HEADERS, timeout=20) as client:
            ...
            try:
                r = await client.get(_NUBANK_API)
                if r.status_code != 200:
                    logger.warning("Nubank returned %d", r.status_code)
                    return jobs
                ...
                logger.warning("Nubank error: %s", exc)

    except Exception as exc:
        logger.warning("Nubank scraper error: %s", exc)

    logger.info("Nubank: %d jobs found", len(jobs))
    return jobs
